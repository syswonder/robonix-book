...
模板中的导航服务和问候技能以本机 Python 进程运行，因此执行 `rbnx build` 和 `rbnx boot` 时都要保持这个虚拟环境处于激活状态。`grpcio-tools` 用于生成 gRPC 类型，`mcp` 和 `fastmcp` 提供模型上下文协议服务器与工具层，`PyYAML` 用于读取配置；缺少这些依赖时，构建或启动会直接报出对应的 Python 导入错误。
...
| 能力约定（Contract） | 稳定、可复用的接口定义，包含数据类型、交互模式、版本和类别 | `robonix/primitive/camera/rgb` |
| 能力（Capability） | 运行实例对某条能力约定的具体实现与传输绑定 | `front_camera` + `robonix/primitive/camera/rgb` + `ros2` |
| 运行实例 | 软件包启动后注册到 Atlas 的具体实例 | `front_camera` |
...
同一运行实例可以用不同传输实现同一能力约定，因此运行时能力键包含：
...
提供方必须显式选择传输。当前 Python 运行时不会在装饰器注册阶段完整校验“模式—传输”矩阵，因此开发者仍需遵守能力约定，并通过端到端测试确认生成代码和消费者一致。

### 4.3 全局与软件包内能力约定

标准能力约定位于 Robonix 源码的 `capabilities/`。只有软件包独有、尚未成为标准接口的能力才放到自身 `capabilities/`。在软件包目录运行 `rbnx codegen` 时，会合并全局能力约定与当前软件包的 `capabilities/`。启动整机部署时，`rbnx boot` 只会自动把 `primitive`、`service` 和 `skill` 列表中软件包的 `capabilities/` 追加到 Atlas。若确实需要加载系统组件的额外目录，可把 `system.atlas.capabilities` 设置为逗号分隔的单个字符串；该字段会**完整替换**自动生成的目录列表，因此必须同时列出全局 `capabilities/`、需要保留的软件包覆盖目录和额外系统目录，建议使用展开后的绝对路径。目录按顺序合并，后加载的同 ID 描述会覆盖先加载的描述，因此不要无意覆盖全局能力约定。IDL 通过相对路径和类型名关联，不应依赖同路径文件覆盖全局 IDL。
...
需要接收 `CMD_INIT`、`CMD_ACTIVATE` 或延迟激活的提供方必须有可用的 `<namespace>/driver` 能力约定。标准命名空间通常复用主仓库中的 driver 约定；自定义命名空间在没有全局约定时需要软件包内 driver TOML。没有 driver 的提供方在监听就绪后会自动进入 `ACTIVE`，但不会收到生命周期配置。

每个提供方应只声明一个 driver。当前调用方选择第一条以 `/driver` 结尾的能力，并不会替开发者消除重复声明。
...
脚手架中的 `main.py` 会生成一个 `@provider.on_init` 处理函数，但空的 `capabilities/` 不包含 `<namespace>/driver` 能力约定。按当前运行时行为，这个处理函数不会被调用，提供方会在监听就绪后直接进入 `ACTIVE`，部署 `config` 也不会送达。若软件包不需要生命周期配置，可以删除该处理函数；若需要配置或 INIT/ACTIVATE 阶段，应添加 `<namespace>/driver` TOML（可复用 `lifecycle/srv/Driver.srv`），重新运行代码生成，并保留 `on_init`。
...
下面的教学服务复用标准导航能力约定，因此不复制全局 TOML。它完整声明生命周期、提交目标、查询状态和取消目标四条能力；示例中的状态表只是可运行骨架，接入真正规划器时必须由规划器回调更新状态。

```bash
rbnx package-new my_navigate --type service
```

将 `package_manifest.yaml` 中的能力列表改为：
...
`name` 必须与 `Service(id="my_navigate", ...)` 一致。清单中恰好一条 `driver` 能力使运行时可以发送 `CMD_INIT`；服务随后进入激活阶段。服务若需要底盘，应在 `on_activate` 中发现并连接目标运行实例：
...
下面的示例导入 `nav_msgs.msg.Odometry` 并发布 ROS 2 数据，因此不是纯 MCP 软件包。构建脚本必须执行 `rbnx codegen -p "$PKG" --mcp --ros2`，再按[构建 ROS 2 叠加工作区](integration-guide/build-and-codegen.md#构建-ros-2-叠加工作区)编译并加载生成的叠加层；目标系统还必须提供所选 ROS 2 发行版的运行环境。Robonix 自有消息和服务类型始终来自这次代码生成的叠加层，不能改用系统中同名的旧接口包。
...
`system` 中 Atlas、Executor、Soma、Vitals、Pilot 和 Liaison 等内置组件由 `rbnx` 转换为各自命令行参数。由 `rbnx` 直接管理的非内置系统软件包和服务会把 `config` 写到 `rbnx-boot/instances/`，仅供调试和事后检查；Soma 管理的原语和技能直接在内存中序列化配置，不生成同类实例文件。两条路径都不会把配置文件路径传给 `rbnx start`，而是在提供方注册后通过 Driver `CMD_INIT` 的 `config_json` 发送配置。没有 driver 的提供方不会收到这份配置。

远程软件包使用 `url`；本地软件包使用 `path`。`manifest` 用于选择该软件包提供的特定平台清单，例如 Jetson 原生构建。当前 `rbnx build -f` 在应用选择器前仍要求软件包根目录存在默认 `package_manifest.yaml`，因此提供平台变体时也应保留默认清单。完整字段见[软件包与部署清单规范](integration-guide/packaging-spec.md)。
...
使用 `--skip-system` 时，`rbnx` 跳过整个 `system` 块，也不会触发由本次启动管理的 Soma 原语/技能阶段；它仍会连接外部 Atlas 并启动清单中的服务。外部系统组件以及原语、技能的启动需由操作方另行保证。

原语、服务和技能各自遵循部署清单中的列表顺序。内置系统组件使用固定顺序 `atlas`、`executor`、`soma`、`vitals`、`pilot`、`liaison`；额外 `system` 项当前存入 `HashMap`，不要依赖它们在 YAML 中的声明顺序。非内置系统软件包或服务的单项失败会被汇总，启动流程可能继续检查其余软件包；不要把“出现启动摘要”当成全部成功，应检查 `failures` 和日志。
...
该命令先读取并解析原部署清单，以便为停止钩子恢复同一组顶层 `env`，再读取 `rbnx-boot/state.json`，执行提供方生命周期关闭、软件包停止钩子，并终止对应启动实例的进程组。当前状态记录没有独立的容器清单；容器化软件包需要在 `stop` 中完成自身的容器清理。因此原部署清单必须仍然存在且可解析。
...
第一个参数可以是 `Capability` 或 `CapabilityProvider`。返回的 `Channel` 包含 `endpoint` 和传输参数，并在 Atlas 中记录消费者到提供方的连接。`Capability.description` 保存自然语言描述；`McpParams` 只包含输入结构 JSON。
...
`create_publisher` 和 `create_subscription` 会立即创建 ROS 对象并尝试向 Atlas 声明能力，应在 provider 注册后调用，通常放在 `on_init`。`declare_ros2_topic` 和 `declare_ros2_service` 只向 Atlas 声明端点；它们不会替开发者创建 `rclpy` 发布者、订阅者或服务。
...
`provider.run()` 完成注册、能力声明、服务监听、心跳和信号处理。退出时运行时把提供方状态设为 `TERMINATED`，关闭通道、受管理子进程和 gRPC 服务；当前 `_teardown()` 不会显式停止进程级 ROS 后端或 FastMCP 的 uvicorn 线程，也不会调用 Atlas unregister。通常由随后发生的进程退出回收这些资源，Atlas 通过状态和心跳回收记录。
...
| `rbnx start [-p <package>]` | 单独启动软件包；恰好声明一条 driver 时先发送 `CMD_INIT`，非技能再发送 `CMD_ACTIVATE`，技能保持 `INACTIVE`；无 driver 时不发送配置或生命周期命令，退出时不执行 manifest 的 `stop` |
...
| 实例 `branch` | 首次克隆远程仓库时使用的分支或标签；不支持任意提交 SHA，已有缓存不会因字段变化自动切换 |
