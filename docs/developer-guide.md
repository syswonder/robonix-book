...
python -m pip install grpcio-tools mcp PyYAML
...
模板中的导航服务和问候技能以本机 Python 进程运行，因此执行 `rbnx build` 和 `rbnx boot` 时都要保持这个虚拟环境处于激活状态。`grpcio-tools` 用于生成 gRPC 类型，`mcp` 提供模型上下文协议服务器，`PyYAML` 用于读取配置；缺少这些依赖时，构建或启动会直接报出对应的 Python 导入错误。
...
Atlas 二进制未配置时绑定 `0.0.0.0:50051`；`rbnx init` 生成的部署清单会显式设置 `127.0.0.1:50051`。服务端监听地址可由部署中的 `system.atlas.listen` 或 `ROBONIX_ATLAS_LISTEN` 修改，能力提供者和 `rbnx` 等客户端则用 `ROBONIX_ATLAS` 选择连接端点。能力提供者启动后先注册自身，再声明能力；消费者可通过 `ATLAS.find_capability(...)` 搜索并建立连接。详细流程见 [Atlas 能力目录](architecture/atlas.md)。
...
`system` 中 Atlas、Executor、Soma、Vitals、Pilot 和 Liaison 等内置组件由 `rbnx` 转换为各自命令行参数。由 `rbnx` 直接管理的非内置系统软件包和服务会把 `config` 写到 `rbnx-boot/instances/`，仅供调试和事后检查；Soma 管理的原语和技能直接在内存中序列化配置，不生成同类实例文件。两条路径都不会把配置文件路径传给 `rbnx start`，而是在提供者注册后通过 Driver `CMD_INIT` 的 `config_json` 发送配置。没有 driver 的提供者不会收到这份配置。
...
该命令先读取并解析原部署清单，以便为停止钩子恢复同一组顶层 `env`，再读取 `rbnx-boot/state.json`，执行 provider 生命周期关闭、软件包停止钩子，并终止对应启动实例的进程组。当前状态记录没有独立的容器清单；容器化软件包需要在 `stop` 中完成自身的容器清理。因此原部署清单必须仍然存在且可解析。
...
`rbnx clean -f robonix_manifest.yaml` 默认保留 `rbnx-boot/cache/`；只有加 `--cache` 才删除远程软件包缓存。单独执行 `rbnx start --config <file>` 时仍会先读取软件包清单，然后在 provider 注册后通过 Driver `CMD_INIT` 发送合并后的配置；配置文件路径不会暴露给 provider 进程。
...
部署清单的相对路径以部署文件所在目录为基准。软件包如何解释 `config` 由其 `config.spec` 和实现共同定义。远程包缓存在 `rbnx-boot/cache/`；修改 `branch` 后需要手动切换缓存中的 checkout，或用 `rbnx clean -f <manifest> --cache` 删除缓存再重新构建。
