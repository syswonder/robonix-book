...
Soma 描述机器人本体与部件，不负责环境物体，也不替 Vitals 做健康阈值判断。`soma.yaml` 是本体描述 sidecar；部署清单里的 `system.soma` 和可选的 Soma `config` 文件是进程配置。不要把监听地址、Atlas 地址等进程参数写进本体文件，也不要把 `robot.components` 写进进程配置。

当前实现有意只解析启动和运行状态所需的最小字段，其余内容原样保留给 Pilot 等消费者。下面的表格区分了**运行时强制约束**和**作者约定**；判断依据是当前 [`store.rs`](https://github.com/syswonder/robonix/blob/181d3eb974fd495a795ed120a0a4c6e6f342d179/system/soma/src/store.rs)，而不是把示例字段误当成完整的类型化 schema。

| 字段 | 当前 Soma 行为 | 接入方责任 |
|---|---|---|
| `urdf.path` | 必填；读取指向的文件。相对路径以 `soma.yaml` 所在目录为基准，绝对路径保持不变 | 提供完整整机 URDF，并用外部工具验证 XML、link、joint 和 TF |
| `robot.id` | 必填且去除首尾空白后不得为空；一个 Soma 进程只服务这一台机器人 | 在部署内保持稳定、唯一，并与调用方使用的 robot ID 一致 |
| `robot.footprint` | 可选；若存在，`base_frame` 必须非空，`points` 至少包含 3 个有限的 `[x, y]` 顶点，并应包围本体原点；Soma 据此计算内切和外接半径 | 使用底盘实际外廓，统一单位为米，不把伸展中的机械臂临时包络混入静态底盘轮廓 |
| `components`、`exports`、`description`、尺寸和质量 | 未作通用类型或引用校验；原始 YAML 会被保留并通过 `get_yaml` 返回 | 维护逻辑部件树，并逐项核对提供方、能力路径、URDF link/joint 与真实部署 |
| 夹爪 `state` | 只在满足特定识别条件时用于运行状态聚合，不是通用 schema 校验器 | 按下文标定 `joint_name`、`open_position_m` 和 `open_tolerance_m` |
...
`id`、`type`、可选的 `urdf_link` / `urdf_joint`、`exports[]` 和嵌套 `components` 构成推荐的部件写法。`provider_id` 应等于部署清单中对应软件包实例的 `name`，每个 `capabilities[].path` 应与该实例实际注册的能力约定完全一致。Soma 当前不会替你检查这些引用；必须在启动后用 Atlas 结果对照。`components` 是面向系统和模型的逻辑树，几何变换仍以完整 URDF/TF 为准，不必把 URDF 的每个关节重复成部件。

夹爪运行状态还需要满足以下条件：

- 部件 `type` 必须是 `parallel_jaw_gripper` 或 `end_effector`。
- 该部件自身或祖先部件的 `exports` 必须把 `robonix/primitive/arm/joint_states` 绑定到正确 `provider_id`。
- `state.open_position_m` 必须是数值；`joint_name` 省略时默认为 `gripper`，`open_tolerance_m` 省略时默认为 `0.002` 米。
- `joint_name` 必须与该提供方的 JointState `name` 完全一致；开启位置和容差应在空夹爪完全张开时实测。

新鲜样本落在容差内时状态为 `open`；否则为 `holding_or_partially_closed`。样本超过 2 秒或缺少对应 joint 时状态为未知。`likely_holding` 只是“没有处于标定开启位置”的启发式结果，不能证明夹爪内确实有物体。
...
`robot_yaml` 的相对路径以部署清单所在目录为基准。若当前 `-f` 选择的清单同目录存在 `soma.yaml`，也可以省略 `robot_yaml`；包含原语或技能的部署会自动注入 Soma 和该 sidecar。显式写出的 `system.soma.config` 也是进程配置路径，不是本体文件。完整的注入、默认值和诊断规则见[系统部署与启动流程](../architecture/deployment-and-startup.md#soma-sidecar-与进程配置)。
...
| footprint 解析失败 | 检查 `base_frame`、至少 3 个有限顶点、顶点顺序和原点是否位于轮廓内 |
...
用于 RGB-D Scene 的提供方必须由同一个 `camera_provider_id` 同时暴露标准 `camera/rgb`、`camera/depth`、`camera/intrinsics` 和兼容期的 `camera/extrinsics` 能力。存在多个相机时必须显式绑定，不能把不同提供方的图像和标定混合。
...
在迁移完成前，相机原语应从同一整机 URDF/TF 读取或派生 `camera/extrinsics`，不要另外维护一套独立标定常量。该消息的 `header.frame_id` 是父坐标系，通常为 `base_link`；`child_frame_id` 是所选相机的光学坐标系，例如 `camera_color_optical_frame`。`transform` 表示 `T(parent ← child)`，即：
