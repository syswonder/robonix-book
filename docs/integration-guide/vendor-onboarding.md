...
Soma 描述的是机器人本体与部件，不负责环境物体或健康判定。最小结构如下；完整字段说明以当前源码的 [`system/soma/README.md`](https://github.com/syswonder/robonix/blob/9a68b2918717c0421faff690eb7bc05e1c8f2a64/system/soma/README.md) 为准。
...
这是 `rbnx package-new` 实际生成的目录。生成的 `main.py` 默认把 provider id 写死为 `acme_chassis`；可复用 package 必须改为读取部署实例名，并为单包调试保留默认值：
...
`rbnx boot` 会把 deployment 条目的 `name` 写入 `RBNX_INSTANCE_NAME`，因此后文的 `base_chassis` 会注册成同名 provider。开始实现后再添加 `config.spec`；只有 provider 需要向 Pilot 说明调用顺序、约束或长任务语义时才添加可选的 `CAPABILITY.md`。`capabilities/` 用于放置尚未进入 Robonix 标准接口的 package-local contract；如果实现的都是标准接口，可以保持为空。
...
每个列表条目代表一个运行实例。`name` 同时是期望注册的 provider id 和日志标签，并通过 `RBNX_INSTANCE_NAME` 传给 package；provider 必须按 4.2 节所示使用该实例名。`url`/`path` 选择 package；`manifest` 选择该 package 为目标平台提供的构建/启动变体；`config` 原样传给 provider 的 `CMD_INIT`。
...
清单中的 `branch: main` 只适合开发阶段跟踪上游。验收与发布时应改用软件包仓库提供的不可变 release tag，并把 `rbnx-boot/cache/` 中每个实际 checkout 的 commit SHA 记录到验收材料；`rbnx` 当前不支持在 `branch:` 中直接填写任意 commit SHA。这样新的克隆才能解析到与验收时相同的软件包内容。
