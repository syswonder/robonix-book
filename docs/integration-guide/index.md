...
创建仓库、构建单个软件包和进入 Robonix 软件包目录（Package Catalog）都只是接入过程中的检查点。只有完整机器人部署在目标本体上完成构建、启动、标准接口与安全验收后，才能在支持列表中标记为“已支持”。
...
前三项用于确认代码可维护、软件包可构建且元数据可被社区工具读取，不能替代整机验收；支持列表只采用最后一项的结果。
