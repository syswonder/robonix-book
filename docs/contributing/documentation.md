...
6. 启动可编辑的本地预览：
   ...
   开发预览中的“最后更新”使用 Docusaurus 的模拟作者和时间，并明确标为模拟值。生产构建才从 Git 提交记录读取真实作者与更新时间；不要根据开发预览中的 `Author` 或 `2018` 判断页面历史。
7. 在提交前运行完整本地检查：
   ...
   该命令先执行 TypeScript 类型检查，再生成 `build/` 生产站点。命令必须以状态码 0 退出。
...
仓库中的 `docs/` 是下一版手册的编辑源。发布一个需要长期维护的 Robonix 版本时，由发布编辑在仓库根目录创建只读快照：

```bash
make version VERSION=<release-version>
```

Docusaurus 会把快照写入 `versioned_docs/` 和 `versioned_sidebars/`。之后的普通修改继续进入 `docs/`；只有修复仍受支持旧版本的问题时，才回补对应快照。不要为尚未发布或没有独立维护周期的分支创建文档版本，否则会产生内容重复和虚假的兼容承诺。

中文是默认语言。增加新语言前，先在 `docusaurus.config.ts` 的 `i18n.locales` 中登记语言，再生成界面消息：

```bash
make translations LOCALE=<locale>
```

界面消息位于 `i18n/<locale>/code.json`，正文翻译位于 `i18n/<locale>/docusaurus-plugin-content-docs/current/`。正文按页面整体翻译；翻译页面必须保留原页面的命令、链接目标和技术含义，并在源码页面变化后同步更新。没有完整正文时，不应把该语言加入公开导航。
