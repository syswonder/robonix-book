...
`metric` 档要跑三个模型，它们决定了下限。下表是容器内实际加载的权重：

| 模型 | 作用 | 权重文件 | 大小 |
|---|---|---|---|
| YOLO-World v8l | 开放词表检测 | `yolov8l-world.pt` | 92 MB |
| MobileSAM | 掩码分割 | `mobile_sam.pt` | 39 MB |
| OpenCLIP ViT-B-32 | 掩码编码 | `open_clip_pytorch_model.bin` | 578 MB |

合计约 710 MB 权重，首次构建时下载。检测器的词表规模见启动日志，Webots 示例是 55 类。

CLIP 换成 ViT-H-14 会显著抬高显存。Replica 基准上实测，同一条流水线：

| CLIP | 峰值显存（alloc / reserved） | 单场景耗时 |
|---|---|---|
| ViT-B-32 | 2.23 / 5.85 GiB | 436 s |
| ViT-H-14 | 5.39 / 9.02 GiB | 538 s |

显存差 2.4 倍、耗时 1.23 倍，而该基准上分割质量没有可测提升，所以默认用 ViT-B-32。

### 实测占用

两组数据，分别来自 x86 开发机和 Jetson AGX Thor：

| 项目 | x86 + RTX 2060 | Jetson AGX Thor |
|---|---|---|
| 显存 | 3.0 GB / 6 GB | 2.23 GiB alloc、5.85 GiB reserved |
| GPU 占用 | 约 37% | 峰值 98% |
| CPU | 约一个核（107%） | — |
| 容器常驻内存 | 启动 1.2 GB，五分钟后 2.8 GB | 与显存共用 122 GB 统一内存 |
| tj 峰值 | — | 47 °C |

x86 一组取自 Webots 办公室场景巡场，`metric` 档、YOLO-World + MobileSAM + CLIP ViT-B-32；Thor 一组取自 Replica 基准单场景跑完。

Thor 是统一内存架构，显存和内存不分开，所以 `nvidia-smi` 的每进程显存显示 `[N/A]`，这是正常的。
