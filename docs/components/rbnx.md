...
```bash
cd robonix && make install     # rbnx、codegen、atlas、executor、soma、vitals、pilot、liaison → ~/.cargo/bin
```

`make install` 末尾会自动执行 `rbnx setup`，把当前克隆的绝对路径写进 `~/.robonix/config.yaml`。此后在磁盘任何位置调用 `rbnx path` 都能解析到源码树内的路径。

只有在 `make install` 报出 setup 警告时，或者要把登记指向另一个 clone 时，才需要手动跑：

```bash
rbnx setup /path/to/another/robonix
```
...
| `rbnx build [-f MANIFEST \| -p PKG \| -g NAME]` | 不带参数时构建当前目录的整个部署；`-p` 构建单个软件包，含 ROS 工作空间 |
| `rbnx clean [--cache]` | 删除构建产物。`--cache` 需配合 `-f MANIFEST` 才会清 `rbnx-boot/cache/` |
...
| `rbnx logs [-d DIR] [-t TAG] [-l LEVEL]` | 读取 Scribe 日志文件并按标签或级别渲染 |
...
| 命令 | 作用 |
|---|---|
| `rbnx init NAME [--path PARENT]` | 初始化一个新的机器人部署目录 |
| `rbnx package-new NAME [-t primitive\|service\|skill]` | 在对应角色目录下创建一个新软件包 |
...
| `rbnx config [-p PATH] [-s]` | 查看或设置软件包存储路径 |
...
`rbnx boot` 按依赖顺序启动清单里声明的内置组件：Atlas、Executor、Soma、Vitals、Pilot、Liaison。清单里没有的组件不会启动，只有一个例外：部署包含原语或技能时会自动补上 Soma。
...
| 变量 | 默认值 | 作用 |
|---|---|---|
| `ROBONIX_DRIVER_INIT_TIMEOUT_S` | `90` | `Driver(CMD_INIT)` 与 `Driver(CMD_ACTIVATE)` 共用的单次 RPC 超时 |

`ROBONIX_DRIVER_INIT_TIMEOUT_S` 是排障工具，不是常规配置。一个组件在 90 秒里初始化或激活不完，通常是被某个外部资源阻塞（例如上一次部署没有完全退出、端口仍被占用），调大它只会把问题推后。先用日志确认时间花在哪一步。
