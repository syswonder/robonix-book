...
三件事最值得先看。**TF 是否连通**：展开 TF 显示，确认 `map → odom → base_link` 这条链存在，断在哪一级就说明哪一级的发布者没起来。**雷达是否贴合<strong>：机器人静止时 LaserScan 的点应当落在墙上，明显偏移说明定位不对。</strong>代价地图是否合理**：机器人周围不应出现大片致命代价，否则规划会失败。
...
按 **`F2`<strong> 开始说话，</strong>停止说话后录音自动结束**，不需要再按一次。对着麦克风说一句和上面同样的话，例如“你前面有什么”。
