...
上表是当前标准能力树的 7 条 `rpc` 约定；内置场景服务使用 MCP 传输。三份场景服务软件包清单列出这 7 条能力，`scene_service/mcp_tools.py` 也有对应处理函数；但当前启动代码只向 Atlas 声明其中 6 条，漏掉了 `get_robot_context`。因此该接口目前不能被 Pilot 或其他消费者发现，不能因为清单和处理函数存在就视为可调用。其处理函数的预期响应包含同一快照中的机器人位姿、所在房间或区域、附近对象与时效状态。
...
输入侧：场景服务使用相机 RGB/depth、相机内参和地图位姿做感知与世界坐标投影，使用 `occupancy_grid` 计算导航安全目标，也会发现并缓存里程计与二维激光雷达输入。当前 RGB-D 路径优先组合 Atlas 发现的地图位姿与 `camera/extrinsics`，只在能力约定路径不可用时回退到 tf2。[Issue #156](https://github.com/syswonder/robonix/issues/156) 提议的目标架构才是以完整 URDF 发布的 TF 作为权威来源、显式 `camera/extrinsics` 仅作兼容回退；该顺序尚未实现。
