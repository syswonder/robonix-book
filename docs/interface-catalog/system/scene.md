...
场景服务维护**世界状态**。它把相机、深度图和视觉语言模型检测结果融合成对象注册表与场景图，用来回答四个问题：屋里有什么、在哪里、彼此是什么关系、去某个对象附近的导航目标是什么。它是规划器向大语言模型提供环境事实的主要来源。

能力约定 TOML 在 `capabilities/system/scene/`。部署配置、内部机制与排障步骤见[场景服务使用指南](../../components/scene.md)，本页只描述接口。
...
内置场景服务通过MCP提供上表 8 条 `rpc` 能力约定。`get_robot_context` 在同一快照中返回机器人位姿、所在房间或区域、附近对象与时效状态，适合在规划前一次获取当前环境上下文。
...
当前实现会监视运行期间的地图身份和 `generation` 变化并自动处理坐标系失效。同一地图的 `generation` 提升意味着坐标原点改变，来自 reset 或重建图。此时 Scene 自动 flush 所有派生对象，重新观测会在新坐标系中重建它们。全部用户房间标记被置为过期（stale）。标记是用户资产，系统只标记、不删除，等用户在地图界面确认仍有效或重新绘制。Mapping 在 Scene 之外切换到另一张地图时，Scene 同样 flush 失效对象，并暂停语义保存。恢复的条件是在地图界面 Load 目标地图完成重新绑定，或重启 Scene。
