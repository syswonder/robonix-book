...
参考实现是 Robonix 源码中的 [`services/voiceprint`](https://github.com/syswonder/robonix/tree/cec06ee874eace27dd622e6ce4685c971f04a9e4/services/voiceprint)。它用 SpeechBrain ECAPA-TDNN embedding 与余弦相似度比对，模型来自 ModelScope 的 `speechbrain/spkrec-ecapa-voxceleb`。注册数据持久化到 `<data_dir>/enrolled.json`，写入通过临时文件和原子替换完成。
