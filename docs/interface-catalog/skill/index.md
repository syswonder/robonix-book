...
开发技能时，每个自定义可调用接口都需要一份能力约定 TOML 和它所需的 IDL 文件。接口由 `rbnx codegen` 生成，目标是 gRPC、MCP 或 ROS 2。`CAPABILITY.md` 强烈推荐，用于向开发者和模型解释技能用途与调用条件，但当前为可选文件；缺少它不会使软件包启动失败。完整流程见[开发者指南](../../developer-guide.md)和[构建与代码生成](../../integration-guide/build-and-codegen.md)。
...
已有技能可以暂时继续使用自己维护的唯一命名空间 Driver 和 Driver TOML。这类技能计划迁移到共享 Driver，迁移前不能同时注册两种 Driver。详见[生命周期兼容流程](../../integration-guide/packaging-spec.md#42-已有命名空间-driver-的兼容流程)。
:::
