...
Robonix 将具身智能运行时拆分为 12 项系统职责。它们是架构边界，不代表每次部署都会启动 12 个进程。这 12 项分属两层：<strong>核心系统组件</strong>和<strong>系统级服务</strong>，区别见下文。本页区分当前实现与规划中的职责，避免把尚未落地的组件写成可用功能。
...
3. **Executor** 验证并执行 RTDL 的 `sequence`、`parallel` 和 `do` 节点。`do` 节点携带 `provider_id` 和 `contract_id`；除 Executor 自身的进程内内建能力（文件操作与 shell）外，模型可调用路径通过 Atlas 连接该提供方的 MCP 能力，并以事件流返回节点状态和结果。Skill 仍为 `INACTIVE` 时，Executor 会在首次调用前通过其 gRPC driver 发送 `CMD_ACTIVATE`。
...
下面两表是组件职责、落地形态和源码位置的**架构摘录**，用于说明边界，不是进程启动清单或完整 API 参考。组件之间的 gRPC、MCP 与 ROS 2 连接关系以[运行时通信](runtime-communication.md)和 Atlas 实际发现结果为准。

两层的区别只有一条判据：**能不能换掉**。

- <strong>核心系统组件</strong>绝对不可更换。它们是运行时的地基，只可能被上层依赖，自身不依赖任何系统级服务。换掉其中任何一个，剩下的系统都无法运行。
- <strong>系统级服务</strong>可以替换，虽然一般不换。它们与导航、建图等部署侧服务处于同一层级，彼此之间存在依赖，只是由 Robonix 自带并放在 `system/` 命名空间下。

源码里两层都位于 `system/`，目录结构目前不体现这个区分。
