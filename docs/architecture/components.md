...
Robonix 将具身智能运行时拆分为 12 项系统职责。它们是架构边界，不代表每次部署都会启动 12 个进程。这 12 项分属两层：**核心系统组件**和**系统级服务**，区别见下文。本页区分当前实现与规划中的职责，避免把尚未落地的组件写成可用功能。
...
- **核心系统组件**绝对不可更换。它们是运行时的地基，只可能被上层依赖，自身不依赖任何系统级服务。换掉其中任何一个，剩下的系统都无法运行。
- **系统级服务**可以替换，虽然一般不换。它们与导航、建图等部署侧服务处于同一层级，彼此之间存在依赖，只是由 Robonix 自带并放在 `system/` 命名空间下。
