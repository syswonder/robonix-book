...
仿真以 bridge 网络运行时，路由器只发布在宿主机 loopback 的映射端口上，跨网络命名空间的 Zenoh 发现不可依赖：宿主侧每个 `rbnx boot` shell <strong>必须</strong>显式设置 `ROBONIX_ZENOH_ROUTER` 为映射端点（即使映射端口就是默认的 `7447`，例如 `tcp/127.0.0.1:7447`）。
