...
- `id`：Atlas 内全局唯一的提供方 ID。对 `rbnx` 管理的 Primitive、Service 和 Skill package，必须与部署项的 `name` 一致；非内置 System package 使用 `system:` 下的键名。
- `kind`：原语、服务或技能。
...
Python provider 框架默认每 30 秒发送一次心跳；Executor、Pilot、Soma、Vitals 和 Liaison 等内置组件当前每 20 秒发送一次。当前 Atlas 默认参数为：
