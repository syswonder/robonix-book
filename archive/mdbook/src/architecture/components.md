...
Robonix 将操作系统职责拆分为 **12 个架构组件**，每个组件在 `system/<name>/` 下有独立目录与 README。这里的 12 个是职责划分，不等于每次部署都会启动 12 个进程。

本页描述的是 **dev-next 分支当前真实的实现状态**——哪些已落地、哪些还是 stub——而不是白皮书的目标蓝图。
...
| **scribe** | 进程内结构化日志 | 库，非独立进程 | Rust（`system/scribe`）；当前无中心日志、查询或回放服务 |
...
`rbnx boot` 以部署清单的 `system:` 配置为准：只启动清单中显式配置的系统组件，并不会无条件启动一套固定进程。当前启动器把 **atlas / executor / soma / vitals / pilot / liaison** 识别为内置 Rust 二进制；其他已配置的 `system:` 项（例如 **scene**）从 `system/<name>/` 解析为 package 后启动。因此，下面的架构组件清单、仓库中的可用实现和某一次部署的实际进程表是三个不同层次。
...
- **scribe**：当前是各进程直接链接的 Rust 日志库，写 stderr 与按 tag 分隔的 JSON Lines 文件；还没有中心 daemon、查询、回放或审计服务。

**chronos / keystone** 仍处于 stub 阶段。Soma 已从 `soma.yaml`、完整 URDF 与 provider 能力读取本体结构，并汇总底盘、关节、夹爪等运行状态；Vitals 消费 Soma 健康流、执行阈值评估，并汇总系统与模块健康。两者职责不同：Pilot 查询 Soma 获取本体事实，查询 Vitals 获取健康判断。
