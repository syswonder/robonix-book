...
每条能力约定由 `capabilities/<kind>/` 下的一份 TOML 加一份 ROS IDL（`capabilities/lib/<…>.msg` / `.srv`）描述，`rbnx codegen` 据此投影到 gRPC / MCP / ROS 2 三种 transport（见 [Package 构建与代码生成](../integration-guide/build-and-codegen.md)）。
...
| 命名空间 | 含义 | 域 |
|----------|------|----|
| `robonix/primitive/*` | 原语：低层设备抽象 | [arm](primitive/arm.md) · [audio](primitive/audio.md) · [camera](primitive/camera.md) · [chassis](primitive/chassis.md) · [health](primitive/health.md) · [imu](primitive/imu.md) · [lidar](primitive/lidar.md) · [robot_description](primitive/robot-description.md) |
...
| `robonix/system/*` | 系统服务：Robonix 自身核心服务 | [pilot](system/pilot.md) · [executor](system/executor.md) · [liaison](system/liaison.md) · [scene](system/scene.md) · [soma](system/soma.md) · [vitals](system/vitals.md) |
