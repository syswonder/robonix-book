...
| 服务（Service） | 标准化算法能力（感知、规划等） | 节点以 `kind: "service"` 注册，提供纯算法能力 | 示例：`rust/examples/nodes/vlm_service.py`、`vla_service.py` |
...
| EAIOS 概念 | 含义 | Robonix 实现 | 关键代码/文档 |
|------------|------|-------------|-------------|
| 控制平面 | 服务注册、发现、channel 协商 | `robonix-server`：gRPC API 提供 `RegisterNode`、`DeclareInterface`、`NegotiateChannel`、`QuerySkillMd` 等 RPC | 代码：`rust/crates/robonix-server/`；设计：[RFC003](../rfc/RFC003-Control-Plane.md) |
| 多传输支持 | ROS2、共享内存、Dora-rs、gRPC 等 | 节点声明支持的传输类型，消费者协商时选定其一，系统分配具体端点；`ridlc --lang arrow` 生成 Arrow 格式 schema 实现跨传输数据兼容 | 代码：`rust/crates/ridlc/src/codegen/arrow_gen.rs`；设计：[RFC006](../rfc/RFC006-Multi-Transport.md) |
| 软硬解耦 | 硬件与算法独立演进 | 原语提供标准化硬件抽象；`ridlc` 从 ROS IDL（`.msg` 文件）生成多格式代码（Rust、Python、Arrow schema）；传输可插拔 | 代码：`rust/crates/ridlc/` |
