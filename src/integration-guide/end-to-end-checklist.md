...
```bash
cd rust
cargo run -p robonix-cli -- validate path/to/your_package
```
...
```bash
cargo run -p robonix-cli -- build -p path/to/your_package
```
...
对 gRPC/ROS 2 传输，`abstract_interface_id`（= namespace + "/" + name）必须在系统接口目录中。检查你的 namespace 和 interface name 拼写。如果你在定义全新的接口，需要先将 IDL 添加到 `rust/robonix-interfaces/lib/`，运行 `ridlc` 生成 proto，然后将 abstract_interface_id 添加到 `robonix-server` 的 `ROBO_SYSTEM_INTERFACE_CATALOG`。
