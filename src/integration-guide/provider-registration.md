...
| `skill_md` | 可选；Skill Node 可在 `RegisterNode` 时附带技能描述文档供 VLM 使用，或在 `skills[]` 列表里显式提交多个 `SkillInfo`。包级别的接口/源码说明放包根目录的 `CAPABILITY.md`，与 skill 注册解耦——参见 [技能库](../skill-library.md) |
