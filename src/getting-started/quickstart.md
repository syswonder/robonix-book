...
凡是向控制平面声明 **`supported_transports` 含 `"mcp"`**、并填写 **`DeclareInterface.listen_port`**（MCP HTTP 数据面端口）的 provider，其 Python 侧必须使用 **`robonix-codegen --lang mcp`** 根据 `rust/crates/robonix-interfaces/lib` 下的 ROS IDL 生成 **`robonix_mcp_types/`**（各 `*_mcp.py` dataclass），与 `rust/contracts` 中的 **`[io]`** 对齐。否则无法 `import` 契约类型，FastMCP 工具签名也无法与契约一致，**不应在缺省生成物的情况下直接跑桥接进程**。

- **推荐**：走包的 **`rbnx build`**（或示例里 `tiago_sim_stack` / `maniskill_vla_demo` / `memsearch_service` 的 `scripts/build.sh`），脚本里会在镜像构建或本地环境准备阶段执行 codegen，并把 **`robonix_mcp_types/`** 与共享包 **`rust/examples/packages/robonix_mcp_contract`**（import 名 **`robonix_mcp_contract`**）加入 **`PYTHONPATH`**（或 `pip install -e` 该包）。
- **工具注册**：MCP 工具请用 **`from robonix_mcp_contract import mcp_contract`** 与 **`@mcp_contract(mcp, contract_id=…, input_cls=…, output_cls=…)`** 注册，**不要**用裸 **`@mcp.tool()`** 手写与契约线格式不一致的参数名（例如 `std_msgs/String` 的 JSON 顶层键为 **`data`**）。线格式与 **`input_cls.to_dict()` / `from_dict()`** 一致；图像类消息在 JSON 中 **`data`** 为 base64 字符串。
- **本地调试**：在仓库 `rust/` 下执行例如：
  ...
  生成目录已列入各包 **`.gitignore`**，勿手抄维护。

完成 codegen 后，再谈 MCP 端口注册与 `rbnx tools` 发现；`./examples/run.sh` 链路上的 **`rbnx build`** 已包含上述步骤（以各包 `build.script` 为准）。
...
> 注意（MCP 必读）：如果 provider 要声明 MCP 接口（`supported_transports=["mcp"]` + `listen_port`），必须在启动前先完成 codegen。当前示例由包的 `scripts/build.sh` 自动执行：先生成 `robonix_mcp_types/`（`robonix-codegen --lang mcp`），再在 package 内准备 `tiago_bridge/proto_gen/`，最后才进行 MCP 端口注册与服务启动。
...
在 TUI 中输入指令即可驱动机器人。**按 Esc** 可向 Pilot 发送 **`AbortSession`**，中断当前一轮推理/工具执行（与 [Provider 注册](../integration-guide/provider-registration.md) 中说明一致）。
...
其中 VLM 在控制平面上只声明 `chat` 接口，但数据面 gRPC 在同一监听端口上同时实现一元 `Chat` 与 server-streaming `ChatStream`（见 `rust/crates/robonix-interfaces/lib/vlm/srv/`）。
...
确认构建阶段已完成 `robonix-codegen --lang mcp` 与 `proto_gen` 生成，再 `rbnx start`。
