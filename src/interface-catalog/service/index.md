...
| 类别 | 是什么 | 谁实现 |
|---|---|---|
| **系统服务** | Robonix 操作系统本体的一部分，构成"用户任务 → 工具执行"的核心管线 | Robonix 项目本身，不可替换 |
...
| Pilot | 推理引擎：任务理解、ReAct 推理循环、会话管理 | [Pilot](pilot.md) |
...
- 推理模型（reason）：接收感知数据与用户任务，进行 CoT 推理与逻辑分析
- 世界模型（world）：预测环境状态变化，辅助任务规划
- 代码模型（code）：生成结构化的 RTDL 方案
