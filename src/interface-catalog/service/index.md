...
默认实现分两类：**上游独立仓库**（`url:` 引入，部署时 clone 到 `rbnx-boot/cache/`，可用 [`rbnx update`](../../integration-guide/build-and-codegen.md#rbnx-update-同步远端-provider) 同步到最新）和 **robonix 仓库内**（`services/` 目录）。每个都带 README / CAPABILITY.md，是该服务的权威部署文档：
...
> 场景/语义地图 [scene](../system/scene.md) 是 **system** 组件（不在 `service/` 域），默认实现在 robonix 仓库 [`system/scene`](https://github.com/syswonder/robonix/blob/main/system/scene/README.md)。
