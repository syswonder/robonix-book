...
所有接口走 Robonix 标准的 `srv/semantic_map/*` 命名空间，IDL 定义在 `capabilities/service/semantic_map/*.v1.toml`。每条服务对应一个内存操作，返回类型都是 Robonix 自己的 message，不是 MJCF 片段。分三组：
...
- schema 版本 v1 的字段确定
- `capabilities/service/semantic_map/*.v1.toml` 写完并跑 `rbnx codegen`
