# `model_inference` Transport（草稿设计 / TODO）
...
引入第四种 data-plane transport：`model_inference`。

| 已有 | 新增 |
|---|---|
| gRPC、ROS 2、MCP、shared_memory | **`model_inference`**（HTTP + 特定 flavor 的 JSON） |
...
DeclareInterface:
  ...
  supported_transports: ["model_inference"]
  metadata_json:
    flavor:       "openai"          # openai | google | anthropic | ollama
...
Atlas 对 `model_inference` transport 不强制 contract id 校验（任何人都可以注册自己的 `robonix/srv/cognition/*`），但至少要验 `metadata_json` 必填字段。
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `flavor` | enum | ✅ | `openai` \| `anthropic` \| `google` \| `ollama`，决定 Pilot 用哪个 SDK |
| `url` | string | ✅ | HTTP endpoint base URL |
| `model_inference` | string | ✅ | 模型 ID（上游命名，如 `gpt-5`、`claude-opus-4-6`、`Qwen2.5-VL-7B`） |
...
同一个节点可以声明多条 `transport=model_inference` interface（不同 model / 不同 capability），Pilot 按 capability 过滤候选。
...
1. **MCP vs model**：MCP tools 本身也是 HTTP，但语义是"工具调用"而不是"模型推理"。保持分开——MCP 走 `transport=mcp`，model 走 `transport=model_inference`。
...
早期设计里有一个 `srv/cognition/reason` gRPC 契约，想把"认知推理"做成独立服务。结合 model transport 的引入重新审视，**这个契约应该废弃**：
...
```
Liaison ──Task──► Pilot ──RTDL Plan──► Executor ──► {MCP, gRPC, Built-in}
                      │
                      │ (Pilot 内部模块)
                      ├── context assembler   : 查 srv/memory/* + srv/common/map/*
                      ├── prompt builder       : 组装 system / tools / history
                      ├── model router         : 查 Atlas transport=model_inference 候选 + 策略选一个
                      └── http client          : 按 flavor 调用 openai/anthropic/google/ollama SDK
                                 │
...
- 迁移期：Pilot 先发现 `model_inference` transport provider；找不到才回退到 gRPC。
