...
```yaml
  - name: scene
    ...
    # 构建期：RBNX_BUILD_TARGET=jetson-native rbnx build -p system/scene
```
...
```bash
# 1) 按目标平台构建每个包（Jetson 例）
RBNX_BUILD_TARGET=jetson-native rbnx build -p system/scene
rbnx build -p services/speech          # uv 包不需要 target，Jetson torch 自愈在 build.sh 内
# 2) 部署 manifest 里给需要 native 的包标平台
#    config: { ROBONIX_SCENE_PLATFORM: jetson_orin }
# 3) 一键拉起整栈
rbnx boot
rbnx caps          # 确认各 provider ACTIVE
```

切回 x86 + docker：构建时不设 `RBNX_BUILD_TARGET`（默认 `x86-docker`）、manifest 里不标 jetson 平台即可。
