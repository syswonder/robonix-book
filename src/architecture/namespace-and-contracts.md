...
- `[contract]`：身份与载荷。`id` 采用 Path-style 命名空间，如 `robonix/primitive/chassis/move`；`version` 为能力约定版本；`kind` 为对应大类；`idl` 指向载荷的 ROS IDL（`.msg` / `.srv`）。**官方 contract**（robonix 源码树的 `capabilities/`）的 IDL 解析到 `capabilities/lib/`；**包内 contract**（package 自己的 `capabilities/`）解析到包的 `capabilities/lib/`。
- `[mode]`：交互形态：
