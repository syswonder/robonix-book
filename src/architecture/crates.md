...
推理与会话运行时（二进制 `robonix-pilot`）。在 Atlas 注册 `contract_id = robonix/srv/pilot`，数据面为 `PilotService.SubmitTask`：请求侧为 `Task`，响应为 `PilotEvent` 流（契约见 `rust/contracts/srv/pilot.v1.toml`、`lib/pilot/`）。

Pilot 内部编排认知大模型（`robonix/srv/cognition/reason`）与 Executor。`rbnx chat` 经 Atlas 按 `robonix/srv/pilot` 发现端点后，通过 `PilotService` 与用户进行流式对话。
...
工具调用执行引擎（二进制 `robonix-executor`）。在 Atlas 注册 `contract_id = robonix/srv/executor`，接收 Pilot 下发的 RTDL **方案**（`Plan`），按工具路由分发调用并流式返回结果。

Pilot 与 Executor 之间传递的是确定性的结构化方案，不含自然语言。Executor 不做推理，专注于分发与执行。
...
| 工具 | 说明 |
|------|------|
| `read_file` | 读取文件内容（如技能 playbook、配置文件） |
| `list_dir` | 列出目录内容 |
| `exec` | 执行 shell 命令 |
| `patch` | 对文件做文本替换 |
...
| `rbnx describe` | 查看节点的技能信息（描述 / playbook 路径 / metadata） |
...
| 命令 | 作用 |
|------|------|
| `rbnx chat` | 启动 TUI 客户端，连接 Pilot（`robonix/srv/pilot`）；当前直连 Pilot 是调试过渡，长期走 Liaison |
| `rbnx graph` | 生成系统拓扑图（内置渲染 PNG/SVG） |

### rbnx chat

`rbnx chat` 通过 Atlas **`QueryNodes(contract_id=robonix/srv/pilot)`** 发现 Pilot 端点，使用 **`PilotService.SubmitTask`** 流式会话，基于 `ratatui` 的终端 UI：
