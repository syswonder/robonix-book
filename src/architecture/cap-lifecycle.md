...
| `RUNNING`    | `Driver(CMD_ACTIVATE)` 返回 `ok=true`；持有热资源，正在对外提供服务。Consumer **必须** 看到此状态才能调用数据接口。 |
...
stateDiagram-v2
    ...
    INITIALIZED --> RUNNING    : CMD_ACTIVATE
    RUNNING    --> INITIALIZED : CMD_DEACTIVATE
    INITIALIZED --> ERROR       : handler 失败
    RUNNING    --> ERROR       : handler 失败
    ERROR       --> INITIALIZED : CMD_DEACTIVATE
    INITIALIZED --> TERMINATED  : CMD_SHUTDOWN / SIGTERM
    RUNNING    --> TERMINATED  : CMD_SHUTDOWN / SIGTERM
...
| `INITIALIZED → RUNNING`（primitive、service） | `rbnx boot` | 紧接 `CMD_INIT` 后自动发 `CMD_ACTIVATE` |
| `INITIALIZED → RUNNING`（system，无 driver contract） | 框架自动 | gRPC + MCP 端口起来即推 `RUNNING` |
| `INITIALIZED → RUNNING`（skill） | Executor | 首次 MCP 调用路由到该 skill 时 |
| `RUNNING → INITIALIZED`（skill） | Executor | 闲置淘汰策略 `EXECUTOR_EVICTION_POLICY=deactivate`（**v0.1 默认 off**，sticky 保持 RUNNING） |
| `→ TERMINATED` | `rbnx shutdown` / SIGTERM / 心跳超时 | 进程退出 / atlas eviction |

唯一的差别在 skill：`rbnx boot` 阶段停在 `INITIALIZED`，进 `RUNNING` 是 Executor 按需触发的；primitive/service/system 在 boot 完成时已经处于 `RUNNING`。
...
sequenceDiagram
    ...
    alt primitive / service
        ...
        C->>A: SetCapabilityState(RUNNING)
        C-->>B: ok
    else skill
...
sequenceDiagram
    ...
    S->>A: SetCapabilityState(RUNNING)
    S-->>E: ok
...
def init(cfg: dict):
    ...
    if not isinstance(sensors, dict) or not sensors:
        return Err("missing sensors block in config")
    # ... use cfg ...
    return Ok()
```
...
| `INITIALIZED → RUNNING` 转换失败 | `rbnx caps --json` 看 `state_detail`；包内日志 grep `CMD_ACTIVATE` |
