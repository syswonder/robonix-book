...
## Config 投递（v0.1.1 invariant）

cap 的 config 只能通过 `Driver(CMD_INIT, config_json)` 进入 cap 进程。**任何 cap 实现不允许从 env / 文件系统读 config**——`rbnx boot` 和 `rbnx start` 都不会向 cap 进程注入 `RBNX_CONFIG_FILE` 或类似变量。

### 流向

```
deploy manifest config: 块  (rbnx boot)
            ↓
            ↓        OR
            ↓
rbnx start --config / --set  (rbnx start)
            ↓
        rbnx CLI 内存里的 JSON
            ↓
        atlas + ConnectCapability(*/driver)
            ↓
        Driver(CMD_INIT, config_json=<JSON>)
            ↓
       cap 进程的 @cap.on_init(cfg) 收到 dict
```

任何写入 `<deploy>/rbnx-boot/instances/<name>.json` 的文件只供 rbnx 自己 + 调试时人工 `cat` 用，cap 进程看不到路径。这条 invariant 让多语言 cap 实现（Python / Rust / Go）只要实现 `Driver` gRPC 服务即可——不需要懂 robonix 的文件约定。
...
@cap.on_init
def init(cfg: dict):
    # cfg 来自 rbnx 推过来的 config_json，已经 JSON-decoded
...
不要在 cap 里读 `os.environ.get("RBNX_CONFIG_FILE")` —— 那条路径在 v0.1.1 已经被 rbnx 主动拒绝注入。
