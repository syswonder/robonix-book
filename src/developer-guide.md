...
```
template_rbnx/
├── robonix_manifest.yaml      ★ 部署入口：rbnx boot 读这个，声明用哪些实例、实例的代码（包）在哪里、实例启动的 config 参数（会传给 @cap.on_init 绑定的函数）
├── primitives/
...
**阻塞主循环**。**绝大多数 `main.py` 最后一行就是 `cap.run()`**：内部先把所有"开门做生意"的步骤跑完（注册 atlas、起 server、起心跳），然后阻塞在 `signal.pause()` 等 SIGTERM/SIGINT；收到信号后 graceful 退出。函数返回意味着进程也退出了。
...
codegen 生成的 dataclass 命名是 `<SrvName>_Request` / `<SrvName>_Response`（按 ROS srv 文件名）；msg 文件直接是 `<MsgName>`。每个实例一个 FastMCP HTTP server（自动选端口），所有 `@cap.mcp` 都挂在同一 server 上。`name=` 可重命名工具（默认用 contract id 末段）。
