...
| `cap.connect_capability(...)` 拿上游 endpoint + 起 ROS subscription | `on_activate` | 一旦 ACTIVE，sub callback 就可能跑业务 |
...
cap = Service(id="my_service", namespace="robonix/service/myorg")
...
def init(cfg: dict):
    ...
    cap_view = caps[0]
    with cap.connect_capability(cap_view,
...
| API | 作用 |
|---|---|
| `cap = Primitive/Service/Skill(id, namespace)` | 在文件顶部构造一个**全局原语 / 服务 / 技能实例**，承载所有装饰器和方法。三个类的方法完全相同（只是 atlas 注册时 kind 不同），按 package 类型选一个就行。`id` 是 atlas 里该实例的唯一 id，`namespace` 固定了之后所有接口 `contract_id` 的前缀（如 `namespace=robonix/primitive/chassis`，则该实例之后声明的接口均以 `robonix/primitive/chassis` 开头） |
...
`Err("...")` 让 cap 进 `ERROR`，`reason` 写入 `state_detail`，`rbnx caps -v` 看得到。完整 atlas 发现 API（按 contract 搜 / 多实例处理）见 §14.4 / §14.5。

### 8.3 上游 connect

`on_activate` 阶段才做"申请热运行资源"——拿上游 endpoint、起 ROS subscription、加载模型、开线程。**按 contract 搜对应的 Capability**：`ATLAS.find_capability(contract_id=..., transport=...)` 返回 `list[Capability]`；`cap.connect_capability(cap_view, contract_id, transport)` 返回一个 `Channel`，里面 `endpoint` 是对方实际地址：
...
在 service / skill 里：你**消费**的 contract 来自上游 Capability → 用 `cap.connect_capability(...)` + 上面 import 的 `chassis_pb2_grpc.RobonixPrimitiveChassisMoveStub`。你**提供**的 contract → 用装饰器 `@cap.mcp(...)` / `@cap.grpc(...)`，签名里类型注解用 `<lib_name>_mcp` 里的 dataclass（MCP）或 `<lib_name>_pb2` 里的消息类（gRPC）。
...
```python
cap = Service(id="my_navigate", namespace="robonix/service/myorg")
```
...
**该做什么**：解析配置、检查依赖（用 `ATLAS.query` / `ATLAS.find_capability` 看上游服务在不在）、声明 contract（不需要——装饰器在 `cap.run()` 内自动注册）。**不要在这里申请热资源**（线程、模型、ROS subs）——那是 `on_activate` 的事。
...
with cap.connect_capability(caps[0],
                            "robonix/primitive/chassis/move",
                            Transport.GRPC) as ch:
    ...
...
with cap.connect_capability(front,
...
**签名**：`cap.connect_capability(cap_view, contract_id, transport) -> Channel`
...
def activate():
    ...
    chassis_ch = cap.connect_capability(cap_view,
                                        "robonix/primitive/chassis/move",
                                        Transport.GRPC)
    return Ok()
...
#### 消费方：`ATLAS.find_capability` + `cap.connect_capability(transport=ROS2)` + rclpy subscription

`cap.connect_capability(...)` 拿到的 `Channel.endpoint` 就是 topic 名（或 service 名），`Channel.params.qos_profile` 是 declare 时给的 QoS preset（service 时为空）。
...
def activate():
    ...
    ch = cap.connect_capability(cap_view,
                                "robonix/primitive/lidar/lidar3d",
                                Transport.ROS2)
    rclpy.init()
